...
class User():
    ...
    def set_roster(self, roster_json: dict):
        """Sets the roster attribute for the user"""
        logger.info(f"Adding current roster information to {self}")
        self.starters = roster_json.get("starters")
        self.ir = roster_json.get("reserve")
        self.players = roster_json.get("players")
        self.season_stats = roster_json.get("settings")
